...
function lejournaldesactus_display_social_links($location = 'header') {
    ...
    // --- Filtrage pour le footer selon le multicheck du Customizer ---
    if ($location === 'footer') {
        $selected = get_theme_mod('lejournaldesactus_footer_socials', array());
        if (is_array($selected) && !empty($selected)) {
            $social_links = array_filter($social_links, function($k) use ($selected) {
